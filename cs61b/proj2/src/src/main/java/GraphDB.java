...
public class GraphDB {
    ...
    public List<String> getLocationsByPrefix(String prefix) {
        ...
        HashSet<Long> idSet = trie.search(prefix);
        for(long a: idSet) {
   ...
   public ArrayList<GraphNode> getLocations(String prefix) {
    ...
    /*initialization*/
        HashSet<GraphNode> close = new HashSet<>(); //can use a bool vector to replace
        //visited
        ...
        PriorityQueue<GraphNode> open = new PriorityQueue<>((o1, o2)-> //queue
    ...
    public GraphNode nearestNode(Point destination) {
        ...
        long id = 0L;
        for (long a : set) { // FIXME: 8/7/16  need a container that holds all the nodes and can iterate
            if (getDistance(nodeinfo.get(a), destination) < distance) {
                distance = getDistance(nodeinfo.get(a), destination);
                id = a;
            }
        }
